...
| `LockedPitchPieces` (molecules) | `PitchHero`, `PitchSection`, `PitchFlow`, `PitchOutcomes`, `EvidenceStack`, `UnlockSteps`, `PitchClose` — the bands both locked pitches are built from. |
...
| `PitchSampleReport` (molecules) | `SampleCaseReport` (coverage headline + stacked bar + legend + case rows) and `SampleFindingReport` (stat tiles + ranked findings with reach) — the two documents the four live tests actually produce. |
| `PitchArt` (molecules) | Eleven flat illustrations (`upload`, `aiplayer`, `analysis`, `report`, `clips`, `compare`, `ask`, `search`, `personas`, `clock`, `rerun`). Token fills, no gradients, one accent element each, `aria-hidden`. |
...
- **Hero** (`PitchHero`) — the focal point: 64px accent gradient tile, an eyebrow naming the group and the footage source ("Human testing · your own recordings" / "AI player testing · 6labs plays your build"), the test name at 32px extrabold with a quiet "Not on your plan" chip beside it, and the promise sentence at 16px. **No buttons** — the ask lives at the foot, after the argument that earns it. The right half carries a `PitchScene`, and **every live test has its own**. It draws the test's setup rather than its report, because the report is already shown twice further down:
...
- **What a finished run looks like** — the sample is the shape of the report that test writes, not one table with a different chip. A **case report** (Functional, AI functional) leads with coverage: "62 of 96 cases verified", a stacked bar and a legend, then unnumbered case rows with `CaseOutcomeTag`. A **findings report** (User, AI behavioural) leads with what the run was made of — the masthead stat tiles — then numbered, ranked findings with `FindingSeverityTag` and the run's own denominator ("6 / 10 sessions" for human, "16 / 20 agents" for AI). Every row on both carries its `▶ N clips`. Each live test has its own `previewTitle` so the masthead is a run name rather than the first row promoted out of the list.
- **How to unlock it** (`PitchClose`) — the three steps between asking and running (mail, with the real address; we enable it here; start where you are), then a brand-tinted footer with the page's one primary "Contact sales", the secondary "See a sample report" and the plan line.
